...
def validate_uuid_string(uuid_str):
    return bool(UUID_REGEX.fullmatch(uuid_str.replace('-', '').lower()))
...
def parse_uuid(uuid_str):
    ...
    if not validate_uuid_string(clean_uuid):
        raise ValueError("Invalid UUID format. Expected 32 hexadecimal characters.")
    return extract_uuid_info(uuid.UUID(hex=clean_uuid))

def uuid_from_high_low(high_input, low_input):
    high = parse_numeric_input(high_input)
    low = parse_numeric_input(low_input)
    for name, val in [("High", high), ("Low", low)]:
        if not (0 <= val < 2**64):
            raise ValueError(f"{name} bits must be a 64-bit unsigned integer.")
    full_int = (high << 64) | low
    return extract_uuid_info(uuid.UUID(int=full_int))
